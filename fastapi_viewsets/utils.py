...
def _resolve_list_options(
    ...
    When the caller did not pass explicit values, the schema's
    ``ListConfig.search_fields`` is used for ``search`` and the
    declarative default ``ListConfig.ordering`` is applied.
    """
